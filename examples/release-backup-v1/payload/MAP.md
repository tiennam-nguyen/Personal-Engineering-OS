...
## Milestone 9 release/recovery topology

- `domain/hardening.py` owns pure inventory, generation, retention, and path rules.
- Application/ports define inward boundaries; filesystem mechanics live in
  `adapters/filesystem/hardening.py`. Domain imports no filesystem, CLI, SQLite, or SDK.
- Backup v1 is a self-contained manifest directory. It includes canonical artifacts, runs, objects,
  protocols, evals, MAP/PLAN/ADRs, and migration records; it excludes SQLite, cache, locks, staging,
  nested backups, quarantine, and `.env`. Create and restore never overwrite.
- Ordered `(path, classification, size, sha256)` generation gates restore, canonical migration, and
  GC. SQLite schema 2 is disposable and rebuildable from canonical artifacts.
- GC only quarantines proven orphans/expired cache after confirmation and a matching verified backup.
- Read-only doctor checks canonical state, runs, objects, index, migrations, protocols/evals,
  qualification sources, secret sentinels, and locks.
- Resurrection: build, inspect, isolated wheel install, restore the synthetic backup, run all three
  compilers, verify runs, destroy/rebuild SQLite, and run final doctor. CI repeats this on Linux.
